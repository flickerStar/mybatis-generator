...
public class XMLMapperGenerator extends AbstractXmlGenerator {
    ...
    protected XmlElement getSqlMapElement() {
        ...
        //xml文件添加<resultMap>标签
        addResultMapWithoutBLOBsElement(answer);
        addResultMapWithBLOBsElement(answer);
        //因为使用公用的selectByExample方法，因此，没有使用这两个方法了，不再生成对应xml
        //xml文件添加<sql id="Example_Where_Clause" >标签组
        //addExampleWhereClauseElement(answer);
        /*//xml文件添加<sql id="Update_By_Example_Where_Clause" >标签组
        addMyBatis3UpdateByExampleWhereClauseElement(answer);*/
        //xml文件添加<sql id="Base_Column_List" >标签组（此处有了修改，因为表名起了别名 t,因此列名前都加了 t.）
        addBaseColumnListElement(answer);
        addBlobColumnListElement(answer);
        //添加SeletctByExample方法标签组（此处有所修改，用了公用的）
        addSelectByExampleWithBLOBsElement(answer);
        addSelectByExampleWithoutBLOBsElement(answer);
        //添加主键查询方法标签组（此处我没有做改变）
        addSelectByPrimaryKeyElement(answer);
        //添加主键删除方法标签组（此处我没有做改变）
        addDeleteByPrimaryKeyElement(answer);
        //添加条件删除方法标签组（此处我没有做改变）
        addDeleteByExampleElement(answer);
        //新增（我觉得不用修改，没做修改，不想生成可以此处注释掉）
        //addInsertElement(answer);
        addInsertSelectiveElement(answer);
        //统计查询数，大多时候配合selectByExample使用
